...
CREATE TABLE tab_size (
    id INT AUTO_INCREMENT PRIMARY KEY,
    size_label VARCHAR(10) NOT NULL -- Example: '38/M', '40/L'
);

-- Junction Table: Mapping Products with Colors
CREATE TABLE product_color (
    id INT AUTO_INCREMENT PRIMARY KEY,
    product_id INT,
    color_id INT,
    FOREIGN KEY (product_id) REFERENCES tab_product(id) ON DELETE CASCADE,
    FOREIGN KEY (color_id) REFERENCES tab_color(id) ON DELETE CASCADE
);

-- Junction Table: Mapping Colors with Sizes
