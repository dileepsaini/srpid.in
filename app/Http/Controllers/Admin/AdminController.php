...
class AdminController extends Controller
{
    ...
    $admins   = Admin::find($adminId);
        ...
        if ($adminId != 1) {
            ...
            $student_count = Student::where('school_id', $adminId)
                ->where('tbl_student.deleted', 0)
                ->count();             $school_count = Admin::where('status', 1)->where('id',$adminId)->count();
        }else{
          
            $school_count = Admin::where('status', 1)->count();

            $student_count = Student::where('tbl_student.deleted', 0)->count();

        }


        $widget['student_count']        = $student_count;
        $widget['school_count']        = $school_count;
